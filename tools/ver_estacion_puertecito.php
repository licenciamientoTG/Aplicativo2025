...
$rows = $db->select("SELECT Codigo, Nombre, Estacion, PermisoCRE FROM Estaciones WHERE Nombre LIKE '%puertecito%'", []);
foreach ($rows as $r) echo "{$r['Codigo']}\t{$r['Nombre']}\t{$r['Estacion']}\t{$r['PermisoCRE']}\n";

echo "--- terminal Aguascalientes ---\n";
$rows2 = $db->select("SELECT id, nombre FROM fuel_terminals WHERE nombre LIKE '%aguascalientes%'", []);
