...
$rows = $db->select("SELECT Codigo, Nombre FROM Estaciones WHERE Nombre LIKE '%clara%' OR Nombre LIKE '%ejercito%' OR Nombre LIKE '%solis%' OR Nombre LIKE '%satelite%' OR Nombre LIKE '%santiago%' ORDER BY Codigo", []);
foreach ($rows as $r) echo "{$r['Codigo']}\t{$r['Nombre']}\n";

echo "--- proveedores petrotal ---\n";
$rows2 = $db->select("
    SELECT t1.id, t2.den
    FROM Proveedores t1
    JOIN SG12.dbo.Proveedores t2 ON t2.cod = t1.id_control_gas
    WHERE t2.den LIKE '%PETROTAL%'
", []);
