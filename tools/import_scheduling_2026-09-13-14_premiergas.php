<?php
/**
 * Import de una sola vez: recepciones de Premiergas (Gaso Mex) del
 * 13/09/2026 y 14/09/2026, capturadas manualmente por el usuario desde el
 * reporte de programación que Abastos maneja fuera de la app.
 *
 * Uso: php tools/import_scheduling_2026-09-13-14_premiergas.php [--commit]
 */
...
function parseFecha(string $raw): ?string {
    $raw = trim($raw);
    if (!preg_match('#^(\d{2})/(\d{2})/(\d{4})$#', $raw, $m)) return null;
    return "{$m[3]}-{$m[2]}-{$m[1]}";
}

// Filas pegadas por el usuario: fecha, litros, producto, estacion (texto libre), horario, transp
$rawRows = [
    ['13/09/2026', 20000, 'Regular', 'jarudo',  '10:00 a. m.', 'PROMEXCO'],
    ['14/09/2026', 20000, 'Regular', 'fuentes', '12:00 p. m.', ''],
];
...
foreach ($rawRows as $i => $row) {
    ...
    if ($fecha === null) {
        $errors[] = "Fila $lineNo: no se pudo interpretar la fecha '$fechaRaw'";
        continue;
    }
