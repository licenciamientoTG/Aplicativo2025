...
$rows = $db->select("
    SELECT s.id, s.hora, s.product, s.litros, s.station_code, s.estatus
    FROM TG.dbo.fuel_reception_schedule s
    WHERE s.supplier_id = 163 AND s.terminal_id = 8
      AND s.fecha = '2026-09-15'
      AND s.station_code = 3
      AND s.estatus <> 'Cancelado'
    ORDER BY s.hora
", []);
foreach ($rows as $r) {
    echo "{$r['id']}\t{$r['hora']}\t{$r['product']}\t{$r['litros']}\t{$r['estatus']}\n";
}
