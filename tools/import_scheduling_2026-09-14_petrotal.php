<?php
/**
 * Import de una sola vez: recepciones de Petrotal (proveedor real, terminal
 * Petrotal) del 14/09/2026, capturadas manualmente por el usuario desde el
 * reporte de programación que Abastos maneja fuera de la app.
 *
 * La columna PROV del reporte (ej. "PREMIER") es una anotación del usuario
 * -- NO cambia el supplier_id (todas estas filas son de Petrotal), se
 * guarda tal cual en el campo "referencia" para no perder el dato.
 *
 * Uso: php tools/import_scheduling_2026-09-14_petrotal.php [--commit]
 */
...
const SUPPLIER_ID = 122; // Petrotal
...
// Filas pegadas por el usuario: litros, producto, estacion (texto libre), horario, transp, prov
$rawRows = [
    [20000, 'Regular', 'clara',    '4:00 p. m.',  'IsaacFZN4539', ''],
    [20000, 'Regular', 'ejercito', '10:00 a. m.', 'IsaacFZN4539', 'PREMIER'],
    [20000, 'Regular', 'solis',    '12:00 p. m.', 'IsaacFZN4539', ''],
    [20000, 'Regular', 'satelite', '10:00 a. m.', 'OmarFZN4539',  'PREMIER'],
    [20000, 'Regular', 'santiago', '12:00 p. m.', 'OmarFZN4539',  'PREMIER'],
];
...
foreach ($rawRows as $i => $row) {
    [$litros, $productoRaw, $estacionRaw, $horaRaw, $transpRaw, $provRaw] = $row;
    $lineNo = $i + 1;
    ...
    $referencia = trim($provRaw) !== '' ? trim($provRaw) : null;
