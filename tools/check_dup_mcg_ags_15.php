...
$rows = $db->select("
    SELECT s.id, s.hora, s.product, s.litros, s.station_code, e.Nombre, s.referencia, s.estatus
    FROM TG.dbo.fuel_reception_schedule s
    LEFT JOIN TG.dbo.Estaciones e ON e.Codigo = s.station_code
    WHERE s.supplier_id = 139 AND s.terminal_id = 8
      AND s.fecha = '2026-09-15'
      AND s.estatus <> 'Cancelado'
    ORDER BY s.station_code
", []);
foreach ($rows as $r) {
    echo "{$r['id']}\t{$r['hora']}\t{$r['product']}\t{$r['litros']}\t{$r['station_code']}\t{$r['Nombre']}\t{$r['referencia']}\t{$r['estatus']}\n";
}
