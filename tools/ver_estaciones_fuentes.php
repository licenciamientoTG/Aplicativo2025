...
$rows = $db->select("SELECT Codigo, Nombre FROM Estaciones WHERE Nombre LIKE '%fuente%' OR Nombre LIKE '%jarudo%' ORDER BY Codigo", []);
foreach ($rows as $r) echo "{$r['Codigo']}\t{$r['Nombre']}\n";

echo "--- terminales ---\n";
$rows2 = $db->select("SELECT id, nombre FROM fuel_terminals ORDER BY nombre", []);
foreach ($rows2 as $r) echo "{$r['id']}\t{$r['nombre']}\n";

echo "--- proveedores (premiergas) ---\n";
$rows3 = $db->select("
    SELECT t1.id, t2.den
    FROM Proveedores t1
