    ...
    function incidentTable(incidents) {
        ...
        incidents.forEach(incident => {
            ...
            html += '<tr><td><a target="_blank" rel="noopener" href="' + mojoUrl(incident.ticket_mojo_id) + '">#' + escapeHtml(incident.ticket_mojo_id) + '</a></td>' +
                '<td>' + escapeHtml(incident.serie_urovo || incident.serial_urovo || '—') + '</td><td>' + escapeHtml(incident.descripcion || '—') + '</td><td>' + formatDate(incident.fecha_apertura_mojo) + '</td><td>' + escapeHtml((incident.dias_laborales ?? incident.dias_habiles ?? 0) + ' días / ' + Number(incident.horas_laborales ?? 0).toFixed(2) + ' h') + '</td>' +
