...
class Operations{
    ...
    function get_sales_in_isle($isle, $tabId, $turno, $fechatabular, $codigoestacion, $limiteFajilla, $islands) {
        ...
        $salesDetail = $this->tabulatorModel->get_sales_tab_detail(
            intval($tabId),
            intval($codigoestacion),
            dateToInt($fechatabular),
            intval($turno),
            strval($isle)
        ) ?: [];
        ...
        foreach ($salesDetail as $row) {
            switch ($row['TipoRegistro']) {
                case 'SUMMARY':
                    $summary = [
                        'Isla' => $row['Isla'],
                        'TotalVentasReportadas' => floatval($row['TotalVentasReportadas']),
                ...
                case 'PAYMENT':
                    $valores[] = [
                        'ValorDescripcion' => $row['ValorDescripcion'],
                ...
                case 'READING':
                    $readings[] = [
                        ...
                        'initialElectronicReading' => floatval($row['initialElectronicReading']),
                        'finalElectronicReading' => floatval($row['finalElectronicReading']),
        ...
        $ventas = "";
        $islandName = $summary['Isla'];

        $ventas .= "<h5 class=\"card-title\">Ventas totales - {$islandName}</h5>";
        ...
        $total_amounts = floatval($summary['TotalVentasReportadas']);
        $sales = "<h5 class=\"card-title\">Formas de pago - {$islandName}</h5><table class=\"table table-sm table-striped table-hover table-payments table1\" style=\"font-size: x-small\"><thead><tr><th>FORMA PAGO</th><th>MONTO</th></tr></thead><tbody>";
        foreach ($valores as $row) {
            $description = $row['ValorDescripcion'] ?: 'Sin descripcion';
            $sales .= "<tr><td class=\"text-nowrap\">{$description}</td><td class=\"text-nowrap\">$ " . number_format($row['Total'], 2) . "</td></tr>";
        }
        $diff = floatval($summary['Diferencia']);
        ...
        $totalVentas = floatval($totals['total_ventas'] ?? 0);
        $totalIngresado = floatval($totals['total_ingresado'] ?? 0);
        $calculo = number_format(($totalVentas - $totalIngresado), 2, '.', ',');
        $diff_tab_span = "Diferencia: $" . $calculo;
        json_output([
            'sales' => $sales,
            'ventas' => $ventas,
            'diff_tab_span' => $diff_tab_span,
            'difference' => $diff,
    ...
    function sales_tab() : void {
            ...
            switch ($row['TipoRegistro']) {
                case 'SUMMARY':
                    $salesSummary[] = [
                        'codisl' => $row['codisl'],
                        'Isla' => $row['Isla'],
                        'TotalVentasReportadas' => $row['TotalVentasReportadas'],
                ...
                case 'READING':
                    $lecturasPorIsla[$row['codisl']][] = [
                        'nrobom' => $row['nrobom'],
                        'Producto' => $row['Producto'],
                        'codprd' => $row['codprd'],
                        'initialElectronicReading' => $row['initialElectronicReading'],
                        'amount' => $row['amount'],
                        'finalElectronicReading' => $row['finalElectronicReading'],
        ...
        $json = [
            'html'  => $this->twig->render($this->route . 'salesTab.html', compact('tabId', 'codigoestacion', 'status', 'fechatabular', 'salesSummary', 'valoresPorIsla', 'lecturasPorIsla', 'LimiteFajilla', 'Total', 'turno', 'exchange_now', 'TotalPending', 'DBString', 'islands'))
        ];
...
// Método para obtener volumen del tanque
    ...
    private function terminalUserCan(string $permission): bool {
        return isset($_SESSION['tg_user']['Id']) && $this->terminalInventoryModel->hasPermission((int)$_SESSION['tg_user']['Id'], $permission);
    }
    private function terminalSettings(): array {
        try { return $this->terminalInventoryModel->getSettings(); }
        catch (Throwable $e) { error_log('No se pudo leer la configuración de inventario de terminales: '.$e->getMessage()); return ['dia_cierre_semana'=>7]; }
    }
    private function terminalWeek(): array {
        $closingDay=(int)($this->terminalSettings()['dia_cierre_semana'] ?? 7);
        if ($closingDay < 1 || $closingDay > 7) $closingDay=7;
        $today=new DateTimeImmutable('today');
        $daysUntilClosing=($closingDay-(int)$today->format('N')+7)%7;
        $closingDate=$today->modify('+'.$daysUntilClosing.' days');
        return [$closingDate->modify('-6 days')->format('Y-m-d'), $closingDate->format('Y-m-d')];
    }
    private function terminalAssignedStation(): array|false {
        $stationId=(int)($_SESSION['tg_user']['IdEstacion'] ?? 0);
        if (in_array($stationId,[0,4,20],true)) return false;
        foreach (($this->estacionesModel->get_select_stations() ?: []) as $station) {
            if ((int)$station['Codigo'] === $stationId) return $station;
        }
        return false;
    }
    ...
    private function syncTerminalIncidents(int $stationId, string $origin): void {
        try {
            ...
            foreach ($this->terminalInventoryModel->activeIncidents($stationId) as $incident) {
                ...
                if (!$service->isOpen($ticket)) $this->terminalInventoryModel->updateTicketState((int)$incident['id'],(string)$incident['estado_mojo'],(string)($ticket['status'] ?? 'closed'),$ticket['solved_on'] ?? date('Y-m-d H:i:s'),$origin);
            }
    ...
    public function terminal_ticket_validate(): void {
        ...
        try { $ticket=(new MojoTerminalTicketsService())->getTicket($ticketId); if (!(new MojoTerminalTicketsService())->validateForType($ticket,$type)) { $this->terminalJsonError('El ticket debe estar abierto y corresponder al formulario indicado.'); return; } json_output(['success'=>true,'ticket'=>$ticket]); } catch (Throwable $e) { $this->terminalJsonError($e->getMessage(),503); }
    }
    public function terminal_ticket_create(): void {
        if (!$this->terminalUserCan(TerminalInventoryModel::CAPTURE_PERMISSION)) { $this->terminalJsonError('Sin autorización.',403); return; }
        $type=(string)($_POST['type'] ?? ''); $description=trim((string)($_POST['description'] ?? '')); $email=trim((string)($_SESSION['tg_user']['Correo'] ?? ''));
    ...
    public function terminal_inventory_save(): void {
        ...
        $stationId=(int)$_SESSION['tg_user']['IdEstacion']; [$weekStart,$weekEnd]=$this->terminalWeek();
        $station=$this->terminalAssignedStation();
        if (!$station) { $this->terminalJsonError('El usuario no tiene una estación válida asignada.'); return; }
        ...
        foreach ($new as $item) {
            $type=(string)($item['type'] ?? ''); $ticketId=(int)($item['ticket_id'] ?? 0); $description=trim((string)($item['description'] ?? ''));
            if (!isset($types[$type]) || !$ticketId || $description==='' || mb_strlen($description)>250 || $this->terminalInventoryModel->ticketUsed($ticketId)) { $this->terminalJsonError('Existe una incidencia nueva inválida.'); return; }
            try { $ticket=(new MojoTerminalTicketsService())->getTicket($ticketId); if (!(new MojoTerminalTicketsService())->validateForType($ticket,$type)) { $this->terminalJsonError('Un ticket no está abierto o no corresponde al tipo de terminal.'); return; }
            } catch (Throwable $e) { $this->terminalJsonError($e->getMessage(),503); return; }
            if ($type!=='urovo' && (!trim($item['provider_folio'] ?? '') || empty($item['provider_date']))) { $this->terminalJsonError('La incidencia de valera requiere datos de proveedor.'); return; }
            $date=$ticket['created_on'] ?? date('c'); $incidentIds[]=$this->terminalInventoryModel->createIncident([$stationId,$type,$ticketId,(string)($ticket['status'] ?? 'open'),date('Y-m-d H:i:s',strtotime($date)),trim($item['provider_folio'] ?? '') ?: null,$item['provider_date'] ?? null,$description,(int)$_SESSION['tg_user']['Id'],(string)$_SESSION['tg_user']['Correo']]);
        }
    ...
    public function terminal_inventory_status(): void {
        ...
        $stations=$this->terminalInventoryModel->stationsInventoryStatus($weekStart);
        ...
        echo $this->twig->render($this->route.'terminal_inventory_status.html',['stations'=>$stations,'completed'=>$completed,'pending'=>$pending,'completionRate'=>$completionRate,'weekStart'=>$weekStart,'weekEnd'=>$weekEnd]);
    }
    public function terminal_inventory_settings(): void {
        ...
        if ($_SERVER['REQUEST_METHOD']==='POST') {
            $day=filter_var($_POST['closing_day'] ?? null,FILTER_VALIDATE_INT);
            if ($day===false || $day<1 || $day>7) { $this->terminalJsonError('Seleccione un día válido para el cierre semanal.'); return; }
            try { $this->terminalInventoryModel->saveClosingDay($day,(int)$_SESSION['tg_user']['Id']); json_output(['success'=>true]); }
        ...
        echo $this->twig->render($this->route.'terminal_inventory_settings.html',['settings'=>$settings,'weekDays'=>[1=>'Lunes',2=>'Martes',3=>'Miércoles',4=>'Jueves',5=>'Viernes',6=>'Sábado',7=>'Domingo']]);
    }
}
