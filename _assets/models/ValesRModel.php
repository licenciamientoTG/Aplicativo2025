    ...
    function get_consumes_by_island_shift($fch, $codgas)
    {
        $query = "
            ...
            FROM [SG12].[dbo].[ValesR] t1
                ...
                LEFT JOIN (SELECT t1.nrotrn, t1.codcli, t1.codgas, t2.tipval, t1.codisl, t1.fchcor, t1.mto FROM [SG12].[dbo].[Despachos] t1 LEFT JOIN [SG12].[dbo].[Clientes] t2 ON t1.codcli = t2.cod WHERE t1.codgas = $codgas AND t1.fchcor = $fch AND t2.tipval IN (3,4)) t4 ON abs(t1.sec) = t4.nrotrn
            WHERE
                t1.fch = $fch
