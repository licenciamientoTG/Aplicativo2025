...
class BudgetModel extends Model{
    ...
    function insertBudgetData($data){
        try{
            ...
            foreach ($data as $row) {
                    $dtBudget = new \DateTime($row['date_budget']);  
                    $dateBudgetStr = $dtBudget->format('Ymd H:i:s');
                    $dateAddedStr = (new \DateTime())->format('Ymd H:i:s');


                    $this->sql->insert($query, [$row['codgas'], $row['budget_monthy'], $row['codprd'],   $dateBudgetStr,
                                        $dateAddedStr, $row['year'], $row['month']]);
            }
            $this->sql->commit();
            return ['success' => true];
        }catch(Exception $e){
