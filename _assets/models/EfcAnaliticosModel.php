...
class EfcAnaliticosModel {
    ...
    public function period(int $stationId, int $year, int $month): array {
        ...
        $stmt=$this->db->prepare("SELECT P.id,P.fecha_reportada,P.hora_original,P.remesa_numero,P.cuenta_mn_original,P.dice_contener_mn,P.real_mn,P.diferencia_mn,P.dice_contener_usd,P.real_usd,P.diferencia_usd,I.nombre_archivo FROM dbo.efc_conc_analiticos_papeletas P JOIN dbo.efc_conc_analiticos_importaciones I ON I.id=P.importacion_id WHERE P.estacion_id=? AND YEAR(P.fecha_reportada)=? AND MONTH(P.fecha_reportada)=? AND I.estado='IMPORTADA' ORDER BY P.fecha_reportada,P.id");
        $stmt->execute([$stationId,$year,$month]); $out=[];
        while($row=$stmt->fetch(PDO::FETCH_ASSOC)) $out[]=['id'=>(int)$row['id'],'date'=>$this->dateValue($row['fecha_reportada']),'time'=>$row['hora_original'],'remittance'=>$row['remesa_numero'],'account'=>$row['cuenta_mn_original'],'declared_mn'=>(float)($row['dice_contener_mn']??0),'real_mn'=>(float)($row['real_mn']??0),'difference_mn'=>(float)($row['diferencia_mn']??0),'declared_usd'=>(float)($row['dice_contener_usd']??0),'real_usd'=>(float)($row['real_usd']??0),'difference_usd'=>(float)($row['diferencia_usd']??0),'file'=>$row['nombre_archivo']];
        return $out;
    }
